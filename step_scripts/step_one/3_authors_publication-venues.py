...
# -*- coding: utf-8 -*-
"""
Step One - 构建 authors 和 publication_venues 表
功能：
1. authors: 创建分区表（按 authorid 范围分区），预估80GB
2. publication_venues: 创建普通表（不分区），预估1GB
3. 解压对应数据集的所有 gz 文件
4. 提取主键字段和完整 JSON 数据
5. 使用 COPY 批量插入
6. 完成后建立索引
...
DATASETS = {
    'authors': {
        'folder': Path(r'D:\2025-09-30\authors'),
        'table': 'authors',
        'dataset_type': DatasetType.AUTHORS,
        'description': 'Authors 数据集',
        'estimated_size_gb': 88,
        'estimated_rows': 110000000,  # 实际1.1亿行
        'partition_size': 200000000,  # 每分区2亿authorid范围，共12个分区，平均~917万行/分区，~7.3GB/分区
        'primary_key': 'authorid',
        'primary_key_type': 'BIGINT',
        'json_field': 'authorid',  # JSON中提取主键的字段名
        'use_partition': True
    },
    'publication_venues': {
        'folder': Path(r'D:\2025-09-30\publication-venues'),
        'table': 'publication_venues',
        'dataset_type': DatasetType.PUBLICATION_VENUES,
        'description': 'Publication Venues 数据集',
        'estimated_size_gb': 1,
        'partition_size': None,  # 不分区
        'primary_key': 'publicationvenueid',
        'primary_key_type': 'TEXT',
        'json_field': 'id',  # JSON中提取主键的字段名
        'use_partition': False
    }
}
...
PARTITION_CONFIG = {
    'min_id': 0,
    'max_id': 2400000000,  # 24亿（authorid 全局范围，ID乱序分布）
}
...
def create_partitioned_table(cursor, dataset_config):
    """
    创建分区表（用于 authors）
    ...
    primary_key_type = dataset_config['primary_key_type']
    ...
    cursor.execute(f"""
        CREATE TABLE {table_name} (
            {primary_key} {primary_key_type} NOT NULL,
            data TEXT NOT NULL
...
def create_normal_table(cursor, dataset_config):
    """
    创建普通表（用于 publication_venues）
...
def build_partition_indexes(cursor, conn, dataset_config):
    """
    为分区表的所有分区建立索引
...
def build_normal_table_index(cursor, conn, dataset_config):
    """
    为普通表建立索引
...
def process_gz_file(gz_path, cursor, conn, dataset_config):
    """
    ...
    """
    table_name = dataset_config['table']
    json_field = dataset_config['json_field']
    primary_key_type = dataset_config['primary_key_type']
                ...
                try:
                    ...
                    if key_value is not None:
                        ...
                        if primary_key_type == 'BIGINT':
                            ...
                            # publicationvenueid 保持字符串
                            key_value = str(key_value)
                        
                        batch_buffer.append((key_value, json_str))
...
def insert_batch(cursor, table_name, data_list):
    """使用 COPY 批量插入数据"""
    buffer = StringIO()
    for key_value, json_data in data_list:
        # 转义特殊字符
        json_escaped = json_data.replace('\\', '\\\\').replace('\n', '\\n').replace('\r', '\\r').replace('\t', '\\t')
        buffer.write(f"{key_value}\t{json_escaped}\n")
    buffer.seek(0)
    
    cursor.copy_from(buffer, table_name)
...
def process_dataset(dataset_name, dataset_config, conn, cursor):
    ...
    use_partition = dataset_config['use_partition']
    
    print("\n" + "="*70)
    print(f"处理数据集: {description}")
    print(f"数据目录: {folder}")
    print(f"目标表: {table_name}")
    print(f"主键字段: {dataset_config['primary_key']} ({dataset_config['primary_key_type']})")
    print(f"分区模式: {'是' if use_partition else '否'}")
    print("="*70)
    ...
    # 创建表
    if use_partition:
        if not create_partitioned_table(cursor, dataset_config):
            print("跳过该数据集")
            return
    else:
        if not create_normal_table(cursor, dataset_config):
            print("跳过该数据集")
            return
    conn.commit()
    ...
    with tqdm(total=len(pending_files), desc="处理进度", unit="file",
        ...
        for idx, gz_file in enumerate(pending_files, 1):
            ...
            records = process_gz_file(gz_file, cursor, conn, dataset_config)
    ...
    # 建立索引
    if use_partition:
        build_partition_indexes(cursor, conn, dataset_config)
    else:
        build_normal_table_index(cursor, conn, dataset_config)
...
def main():
    """主函数"""
    print("="*70)
    print("Step One - 构建 authors 和 publication_venues 表")
    print(f"批次大小: {BATCH_SIZE:,}")
    print(f"数据类型: 主键 + data (TEXT)")
    print("="*70)
    
    # 选择要处理的数据集
    print("\n可用数据集:")
    for idx, (name, config) in enumerate(DATASETS.items(), 1):
        partition_info = f"分区表，{config['estimated_size_gb']}GB" if config['use_partition'] else f"普通表，{config['estimated_size_gb']}GB"
        print(f"  {idx}. {config['description']} ({name}) - {partition_info}")
